...
class WhaleDataModule(LightningDataModule):
    ...
    def val_dataloader(self):
        if self.cfg.n_splits == -1:
            # Return empty dataloader instead of None for PL 2.x compatibility
            return DataLoader([], batch_size=self.cfg.batch_size)
        # return DataLoader(
    ...
    def _should_reload_val_dl(self):
        return False
...
class SphereClassifier(LightningModule):
    ...
    def on_test_epoch_end(self):
        outputs = self.all_gather(self._test_outputs)
        self._test_outputs = []
...
def train(
    ...
    if not ckpt_exists or not args.load_snapshot:
        ckpt_path = None

    if not args.skip_training:
        if ckpt_path is not None and args.load_weights_only:
            # Load only weights, start training fresh (no optimizer/scheduler restore)
            checkpoint = torch.load(ckpt_path, map_location="cpu")
            model.load_state_dict(checkpoint["state_dict"])
            print(f"Loaded weights only from {ckpt_path}, training from epoch 0")
            trainer.fit(model, ckpt_path=None, datamodule=data_module)
        else:
            # Full resume (weights + optimizer + scheduler + epoch) or fresh start
            trainer.fit(model, ckpt_path=ckpt_path, datamodule=data_module)
    elif ckpt_path is not None:
        checkpoint = torch.load(ckpt_path, map_location="cuda")
        model.load_state_dict(checkpoint["state_dict"])
        model = model.cuda()
        print(f"Loaded weights from {ckpt_path} (skip_training mode)")
    else:
        raise ValueError("--skip_training requires --load_snapshot and an existing last.ckpt")
